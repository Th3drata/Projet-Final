...
def walk_sound():
    if not pygame.mixer.get_busy():  # Vérifie si un son est déjà en cours
        # Ajouter une légère variation du volume
        volume = random.uniform(0.4, 0.6)
        walk_sound_effect.set_volume(volume)
        
        # Jouer le son avec un léger décalage aléatoire pour plus de réalisme
        walk_sound_effect.play(maxtime=random.randint(300, 500))  
...
class Button:
    def __init__(self, x, y, width, height, text, text_color, font, color, hover_color, shadow_color=(0, 0, 0), border_radius=10):
...
def afficher_parametres(surface, slider_value, is_dragging):
    ...
    pygame.draw.circle(surface, (0, 200, 0), 
                       (point_x, bar_y + bar_height // 2), int(bar_height * 0.6))
    pygame.draw.circle(surface, (0, 255, 0), 
                       (point_x, bar_y + bar_height // 2), int(bar_height * 0.5))
...
def start_game(screen, font, background_image, dimensions_possibles):
    ...
    display = (dimensions_possibles[-2]) # Récupérer les dimensions de l'écran et mettre une taille en dessous de la plus haute possible
    ...
    while True:
        ...
        for event in pygame.event.get(): # Pour chaque événement Pygame
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                ...
                pygame.mouse.set_visible(menu_open)  # Afficher le curseur si menu ouvert
        ...
        if menu_open:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if bar_x <= mouse_x <= bar_x + bar_width and bar_y <= mouse_y <= bar_y + bar_height:
                    ...
                    slider_value = max(70, min(150, slider_value))  # Clamp entre 70 et 150
